"""
EVM contract helpers: deployment info, DelegateProxyCaller ABI, and contract instance.
"""
...
STAKE_WRAP_ARTIFACT_PATH = os.path.join(  # Kept name for backward-compat
    PROJECT_ROOT,
    "artifacts",
    "contracts",
    "DelegateProxyCaller.sol",
    "DelegateProxyCaller.json",
)
...
def get_stake_wrap_abi(project_root: Optional[str] = None) -> Optional[List[Dict]]:
    """
    Load DelegateProxyCaller ABI from Hardhat artifact (cached after first load).
    Function name kept for backward compatibility.
    Returns None if artifact is missing.
    """
    global _abi_cache
